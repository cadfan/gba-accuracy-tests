...
def get_expected_hashes(refs: dict, test_id: str, bios_mode: str | None = None) -> list[dict]:
    """Get valid reference entries for a test, optionally filtered by BIOS mode.

    Returns list of dicts with hash, tier, emulator, bios_mode.
    If bios_mode is specified, only returns refs matching that mode.
    Schema v1 refs without bios_mode are treated as "hle".
    """
    ...
    for e in entries:
        if "hash" not in e:
            continue
        ref_bios = e.get("bios_mode")
        if ref_bios is None:
            # Schema v1 migration: no bios_mode field means HLE
            prov = e.get("provenance", {})
            ref_bios = prov.get("bios_mode", "hle")
        if bios_mode is not None and ref_bios != bios_mode:
            continue
        result.append({
            "hash": e["hash"],
            "tier": e.get("tier", "candidate"),
            "emulator": e.get("provenance", {}).get("emulator", "unknown"),
            "bios_mode": ref_bios,
        })
    return result


def get_expected_hash_values(refs: dict, test_id: str, bios_mode: str | None = None) -> list[str]:
    """Get just the hash strings for backward compatibility."""
    return [e["hash"] for e in get_expected_hashes(refs, test_id, bios_mode)]
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    runner = None
    cmd_template = None
    bios_mode = getattr(args, "bios_mode", None)
    ...
    results = []
    total_pass, total_fail, total_skip, total_error = 0, 0, 0, 0

    bios_label = f" (bios: {bios_mode})" if bios_mode else ""
    print(f"\ngba-accuracy-tests v0.2.0{bios_label}\n")
    ...
    for suite_dir in suites:
        ...
        for i, test in enumerate(tests, 1):
            ...
            if not rom_path.exists():
                print(f"    [{i}/{len(tests)}] {test_id:<30} SKIP   ROM not found")
                total_skip += 1
            ...
            ref_entries = get_expected_hashes(refs, test_id, bios_mode)

            if not ref_entries:
                print(f"    [{i}/{len(tests)}] {test_id:<30} SKIP   No references" +
                      (f" for bios={bios_mode}" if bios_mode else ""))
                total_skip += 1
            ...
            if matched:
                ...
                if ref_bin_dir.exists():
                    for ref_entry in ref_entries:
                        emu = ref_entry["emulator"].lower().replace(" ", "-")
                        rbios = ref_entry.get("bios_mode", "hle")
                        ref_bin = ref_bin_dir / f"{emu}-{rbios}-{test_id}.bin"
                        if ref_bin.exists():
                            try:
                                expected_raw = ref_bin.read_bytes()
                                diff_path = OUTPUT_DIR / f"{test_id}.diff.png"
                                generate_triptych(expected_raw, actual_raw, diff_path)
                                diff_generated = True
                            except Exception:
                                pass
                            break
                ...
                print(f"    [{i}/{len(tests)}] {test_id:<30} FAIL   hash mismatch ({elapsed:.1f}s)" +
                      (" [diff saved]" if diff_generated else ""))
                if hint:
                    print(f"             Hint: {hint}")
                total_fail += 1
                results.append({
                    ...
                    "actual_hash": actual_hash,
                    "expected_hashes": ref_hash_values,
                    "time_s": round(elapsed, 1),
                })
    ...
    with open(results_path, "w") as f:
        json.dump({
            ...
            "summary": {"pass": total_pass, "fail": total_fail, "skip": total_skip, "error": total_error},
            "results": results,
        }, f, indent=2)
...
def main(argv: list[str] | None = None) -> int:
    ...
    run_p.add_argument("--bios-mode", "-b", choices=["official", "hle", "skip", "cleanroom"],
                       help="BIOS mode for reference filtering (default: match all)")
